<x-app-layout>
    <div class="py-12 bg-gray-900 min-h-screen">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg p-8 text-white">
                ...
                <form action="{{ route('contents.update', $content->id) }}" method="POST" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <input type="text" id="title" name="title" value="{{ old('title', $content->title) }}" class="mt-1 block w-full bg-gray-700 border border-gray-600 rounded-md shadow-sm text-white">
                    </div>

                    {{-- Description --}}
                    <div>
                        <label for="description" class="block text-sm font-medium">Description</label>
                        <textarea id="description" name="description" rows="4" class="mt-1 block w-full bg-gray-700 border border-gray-600 rounded-md shadow-sm text-white">{{ old('description', $content->description) }}</textarea>
                    </div>

                    {{-- URL --}}
                    <div>
                        <label for="url" class="block text-sm font-medium">URL</label>
                        <input type="text" id="url" name="url" value="{{ old('url', $content->url) }}" class="mt-1 block w-full bg-gray-700 border border-gray-600 rounded-md shadow-sm text-white">
                    </div>

                    {{-- Category (bitta) --}}
                    <div>
                        <label for="category_id" class="block text-sm font-medium">Category</label>
                        <select name="category_id" id="category_id" class="mt-1 block w-full bg-gray-700 border border-gray-600 rounded-md text-white" required>
                                ...
                                <div class="flex items-center gap-2 mt-1">
                                    <select name="authors[]" class="bg-gray-700 border border-gray-600 text-white rounded-md w-full">
                                        ...
                                        @foreach($authors as $id => $name)
                                            <option value="{{ $id }}" {{ $author->id == $id ? 'selected' : '' }}>{{ $name }}</option>
                                        @endforeach
                                    </select>
                                    <button type="button" class="px-3 py-1 bg-red-600 text-white rounded hover:bg-red-700 remove-author">-</button>
                                </div>
                            @endforeach
                        </div>
                    ...
                    <div>
                        ...
                        <div id="selectGenres">
                            @foreach($content->generes as $genre)
                                <div class="flex items-center gap-2 mt-1">
                                    ...
                                    <button type="button" class="px-3 py-1 bg-red-600 text-white rounded hover:bg-red-700 remove-genre">-</button>
                                </div>
                            @endforeach
                        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            function addClone(buttonId, containerId, removeClass) {
                ...
                // old selectlar uchun ham remove ishlashi uchun
            ...
            addClone('addAuthorButton', 'selectAuthors', 'remove-author');
            addClone('addGenreButton', 'selectGenres', 'remove-genre');
        });
    </script>
</x-app-layout>
